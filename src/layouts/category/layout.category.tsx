...
export default function LayoutCategory() {
  ...
  const { data, isPending } = useQuery({
    queryKey: ["benefit-list", search, pageNumber, dispense, dateParam],
    queryFn: () =>
      handleFetch({
        label: search,
        number: pageNumber,
  ...
  const handleUpdateStatus = async (id: string, status: boolean) => {
    setInfo(
      Object.assign({}, info, {
        ...
        mode: "status",
        title: status
          ? "¿Esta seguro que desea activar esta categoría?"
          : "¿Esta seguro que desea desactivar esta categoría?",
        status: status,
      })
    );
    onOpenModal();
